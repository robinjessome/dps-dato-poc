 ...
 *
 * Please be aware, this project utilizes an extremely basic cache invalidation
 * technique: by using the `next.tags` option, all requests to DatoCMS are
 * tagged with "datocms" in the Next.js Data Cache. Whenever DatoCMS notifies us
 * of any updates via webhook, we invalidate all requests with the same tag.
 *
